...
<head>
    ...
    <style>
        ...
        * { box-sizing: border-box; }
        ...
        .pix-actions { display:flex; gap: 10px; flex-wrap: wrap; }
        .pix-actions .btn { width:auto; }
    </style>
</head>
<body>
<?php
...
<div class="app">
    <header class="topbar">
        <div class="topbar-inner">
            <div class="brand">
                <div class="brand-logo" title="Logo (base64) carregada do localStorage (playground_logo_base64)">
            ...
            <aside>
                ...
                <div class="card">
                    ...
                    <div class="pix-box">
                        ...
                        <div class="small">Payload (copia e cola)</div>
                        <textarea id="pixPayload" rows="6" spellcheck="false"></textarea>

                        <div class="pix-actions">
                            <button class="btn primary" id="copyPixPayload" type="button">Copiar payload</button>
                            <button class="btn" id="savePixPayload" type="button">Salvar payload</button>
                        </div>

                        <div class="small">
                            Dica: o QR exibido é controlado por <span class="kbd">playground_pix_qr</span> (localStorage) ou pelo placeholder padrão no arquivo.
                        </div>
                    </div>
                </div>
            </aside>
...
<script>
    ...
    const DEFAULT_PIX_PAYLOAD = '00020126580014br.gov.bcb.pix0136c1ae4730-0eae-46cd-af3f-b837e7c5270f5204000053039865802BR5925WALLACE KLEITON COSTA DE 6015SANTO ANTONIO D62070503***6304990E';
    ...
    function initDefaultsOnce() {
        ...
        if (!localStorage.getItem('playground_pix_payload')) {
            localStorage.setItem('playground_pix_payload', DEFAULT_PIX_PAYLOAD);
        }
    }

    function renderPix() {
        const qr = localStorage.getItem('playground_pix_qr') || DEFAULT_PIX_QR_PLACEHOLDER;
        pixQr.src = qr;

        const payload = localStorage.getItem('playground_pix_payload') || DEFAULT_PIX_PAYLOAD;
    ...
    document.getElementById('copyPixPayload').addEventListener('click', async () => {
        await copyText(pixPayload.value || '');
    });

    document.getElementById('savePixPayload').addEventListener('click', () => {
        localStorage.setItem('playground_pix_payload', (pixPayload.value || '').trim());
    });
