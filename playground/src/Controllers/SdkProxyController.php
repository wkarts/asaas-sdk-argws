...
final class SdkProxyController extends AbstractController
{
    ...
    public function call(ServerRequestInterface $request, ResponseInterface $response, array $args): ResponseInterface
    {
        ...
        //
        // ✅ PATCH AQUI:
        // - Se vier "id" no JSON simples (ex.: {"id":"cus_123","name":"X"}):
        //   considera "id" como pathParams (args[0]) e remove do payload (args[3]).
        //   Isso resolve update/delete/getById/cancel etc. sem exigir args complexo.
        if (!array_key_exists('args', $payload) && $payload !== []) {
            ...
            // ✅ Se vier "id" no body simples, assume que é path param (args[0])
            $pathParams = [];
            if (is_array($directPayload) && array_key_exists('id', $directPayload) && (is_string($directPayload['id']) || is_int($directPayload['id']))) {
                $pathParams['id'] = (string) $directPayload['id'];
                unset($directPayload['id']);
            }

            $payloadOnly = (is_array($directPayload) && $directPayload !== []) ? $directPayload : null;
        ...
        // (mantém args[0]/args[1]/args[2] intactos)
